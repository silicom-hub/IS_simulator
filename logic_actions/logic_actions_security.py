...
def zeek_installation(instance, verbose=True):
    """ Install zeek package """
    install(instance, {"module":"zeek"}, verbose=False)
    if verbose:
        print(Fore.GREEN + "      Zeek has been installed!" + Style.RESET_ALL)
...
def suricata_installation(instance, verbose=True):
    ...
    execute_command(instance, {"command":["touch", "/etc/suricata/rules/suricata.yaml"], "expected_exit_code":"0"}, verbose=False)
    if verbose:
...
def fail2ban_installation_configuration(instance, arg, verbose=True):
    ...
    for service in arg["services"]:
        if service["name"] == "cyrus-imap":
            custom_jail_conf.write("[cyrus-imap] \n")
            custom_jail_conf.write("enabled = true \n")
            custom_jail_conf.write("port = imap,imaps \n")
            custom_jail_conf.write("logpath = %(syslog_mail)s \n")
            custom_jail_conf.write("backend = %(syslog_backend)s \n")
            if service["bantime"] != "":
